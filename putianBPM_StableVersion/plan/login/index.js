(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control0_N7zrZx: function (elem) {
      'use strict';
      if (elem) {
        var data = [];var ipts = elem.querySelectorAll('.mini-textbox-input');var btnVal = elem.querySelectorAll('.log');data.push(ipts[0].value);data.push(ipts[1].value);data.push(btnVal[0].value);if (elem.querySelector("#userId").querySelector(".mini-errorIcon")) {
          data.push(true);
        } else {
          data.push(false);
      ...
      if (data.eventType == 'change') {
        ...
        try {
          if (ysp.appMain.isAndroid()) {
            var loginName = yspUser.getUsername('http://192.168.220.51:8000/ptsoa/coframe/auth/login/login.jsp#');var passWord = yspUser.getPassword('http://192.168.220.51:8000/ptsoa/coframe/auth/login/login.jsp#');var timer = setInterval(function () {
              if (elem && elem.querySelectorAll('.mini-textbox-input').length > 0 && elem.querySelector('.log')) {
                elem.querySelectorAll('.mini-textbox-input')[0].value = loginName;elem.querySelectorAll('.mini-textbox-input')[1].value = passWord;elem.querySelector('.log').click();clearInterval(timer);
              }
            }, 1000);
          } else if (ysp.appMain.isIOS()) {
            top.loginBPM = function (username, password) {
              alert("调用成功!");var timer = setInterval(function () {
                if (elem && elem.querySelectorAll('.mini-textbox-input').length > 0 && elem.querySelector('.log')) {
                  elem.querySelectorAll('.mini-textbox-input')[0].value = username;elem.querySelectorAll('.mini-textbox-input')[1].value = password;elem.querySelector('.log').click();clearInterval(timer);
                }
              }, 1000);
